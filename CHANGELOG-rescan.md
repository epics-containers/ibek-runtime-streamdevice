...
90 changed, 3 removed (104 rows). Folders not listed are
byte-identical to `main`. **Source** is the DLS module and release the folder is
imported from. **What changed** is relative to `main`; **Why** names the rule or
decision behind it; **Flag** is what to look at: a user call still pending, a template
marked as needing more than the generic image, an upstream defect carried as-is, a
patch or trim of an upstream file, or a change that breaks an existing `ioc.yaml`.
...
- **Other parameters removed** (each used only in template comments; all pending a user
  call except `IPADDR`, whose asyn port the instance creates itself):
...
| `agilentTurboPump` | CHANGED | agilentTurboPump 2-1 | `name` param removed (`twisTorr305FS`, `twisTorr305IC`); databases args give comment-only macros an empty value (`name`) | no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
| `alicatGasFlow` | CHANGED | alicatGasFlow 2-15 | docs/ (1 file); sim/ (3 files); test/ (1 file); +3 .req; `name` param removed (`alicatGasFlow`, `alicatMassCtrl`, `alicatPressureCtrl`); databases args give comment-only macros an empty value (`DESC`, `name`); source 2-14 -> 2-15: `getGasLong` protocol, `GASNAMEPROTOCOL` param (default `getGas`) | newer prod release; publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; autosave tags in the templates; no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
...
| `caenN1470` | CHANGED | caenN1470 1-4 | docs/ (1 file); +2 .req; `name` param removed (`_caenN1470`); databases args give comment-only macros an empty value (`gda_desc`, `gda_name`) | publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; autosave tags in the templates; no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
...
| `cognexDataMan100` | CHANGED | cognexDataMan100 1-2 | docs/ (1 file); `name` param removed (`dataman100`); databases args give comment-only macros an empty value (`name`) | publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
...
| `granvillePhillips` | CHANGED | granvillePhillips 2-10 | +10 template(s); +entities `gp307Gauge`, `gp307Hcg`, `gp307HcgDummy`, `gp307Pirg`, `gp307PirgDummy`, `gp350Gauge` +4 more; sim/ (2 files); databases args give comment-only macros an empty value (`gctrl`) | release templates not covered on main; publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; no `macLib: macro ... is undefined` at boot | - |
| `gssExplorIR` | CHANGED | gssExplorIR 1-2 | `name` param removed (`explorir`); `explorir` params removed: `SCAN` | no template uses `name` (hard rule); used only in template comments | consumer break: `name`; pending user: dropped `SCAN` (consumer break) |
| `harvardSyringe` | CHANGED | harvardSyringe 1-12 | sim/ (1 file); `name` param removed (`harvardSyringe`); `harvardSyringe` params removed: `desc`; 2 `DESC` strings shortened to 40 characters; header lists the change | EPICS 7 rejects longer `DESC`; publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; no template uses `name` (hard rule); used only in template comments | scripted trim of the upstream template; consumer break: `name`; pending user: dropped `desc` (consumer break) |
...
| `keithley2400` | CHANGED | keithley2400 1-23 | docs/ (1 file); +2 .req; `name` param removed (`Keithley2400`); databases args give comment-only macros an empty value (`R`, `name`) | publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; autosave tags in the templates; no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
...
| `KeithleyDMM6500` | CHANGED | KeithleyDMM6500 1-1 | docs/ (1 file); `name` param removed (`dmm6500`); databases args give comment-only macros an empty value (`name`) | publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
...
| `knauer` | CHANGED | knauer 0-6 | +1 template(s); +entities `ConnectionManagement`; docs/ (1 file); sim/ (2 files); `name` param removed (`Azura21S`, `BlueShadow40P`); databases args give comment-only macros an empty value (`device`, `valve`); `docs/doxygen/manual` without its internal share path | release templates not covered on main; publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
...
| `lakeshore218` | CHANGED | lakeshore218 2-9 | +1 .req; `name` param removed (`lakeshore218`); `lakeshore218` params removed: `lakeshore218Path`; databases args give comment-only macros an empty value (`lakeshore218Path`) | autosave tags in the templates; no template uses `name` (hard rule); used only in template comments; no `macLib: macro ... is undefined` at boot | consumer break: `name`; pending user: dropped `lakeshore218Path` (consumer break) |
...
| `laudaRE2xx` | CHANGED | laudaRE2xx 2-9-9 | +1 template(s); +1 protocol file(s); +entities `lRE2xx_RS485`; sim/ (1 file); +5 .req; `name` param removed (`lIntegralT`, `lMCxxx`, `lRE2xx`, `lVariocool`); `lIntegralT` params removed: `DESC`; `lMCxxx` params removed: `DESC`; `lRE2xx` params removed: `DESC`; `lVariocool` params removed: `DESC`; databases args give comment-only macros an empty value (`DESC`) | release templates not covered on main; publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; autosave tags in the templates; no template uses `name` (hard rule); used only in template comments; no `macLib: macro ... is undefined` at boot | consumer break: `name`; pending user: dropped `DESC` (consumer break); pending user: dropped `DESC` (consumer break); pending user: dropped `DESC` (consumer break); pending user: dropped `DESC` (consumer break) |
...
| `omegaIR2C` | CHANGED | omegaIR2C 1-5 | `name` param removed (`omegaIR2C`); databases args give comment-only macros an empty value (`name`) | no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
...
| `PIpiezo` | CHANGED | PIpiezo 1-23 | +4 template(s); +entities `E871-absGrp`, `E871-relGrp`, `PI-E516`, `actuatorcallback`; docs/ (1 file); sim/ (3 files); +5 .req; `name` param removed (`E725`, `E816`, `E871`); `E816` params removed: `DESC`; databases args give comment-only macros an empty value (`DESC`, `R`, `gda_desc`, `gda_name`); provenance header | release templates not covered on main; publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; autosave tags in the templates; no template uses `name` (hard rule); used only in template comments; no `macLib: macro ... is undefined` at boot | marked non-image: `E871-absGrp.template`, `PI-E516.template` (motor record); upstream: `C877` `REF:MODE` has a stream INP but no DTYP; consumer break: `name`; pending user: dropped `DESC` (consumer break) |
| `pr4000` | CHANGED | pr4000 3-2 | docs/ (1 file); `name` param removed (`pr4000`); `pr4000` params removed: `ch1pv`, `ch2pv`; databases args give comment-only macros an empty value (`ch1pv`, `ch2pv`, `domain`) | publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; no template uses `name` (hard rule); used only in template comments; no `macLib: macro ... is undefined` at boot | consumer break: `name`; pending user: dropped `ch1pv`, `ch2pv` (consumer break) |
...
| `specsVCU1000` | CHANGED | specsVCU1000 1-9-2 | docs/ (3 files); sim/ (1 file); `name` param removed (`specsVCU1000`); databases args give comment-only macros an empty value (`name`) | publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
| `SQC-310` | CHANGED | SQC-310 0-2 | docs/ (2 files); `name` param removed (`SQC310`); databases args give comment-only macros an empty value (`name`) | publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; no template uses `name` (hard rule); no `macLib: macro ... is undefined` at boot | consumer break: `name` |
| `stanford` | CHANGED | stanford 2-7 | +1 template(s); +entities `dummy_stanford_ai`; sim/ (1 file); +1 .req; `sr570` params removed: `DESC`; databases args give comment-only macros an empty value (`DESC`) | release templates not covered on main; publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; autosave tags in the templates; used only in template comments; no `macLib: macro ... is undefined` at boot | pending user: dropped `DESC` (consumer break) |
...
| `tdklambda` | CHANGED | tdklambda 1-5 | `name` param removed (`Gen3300w`) | no template uses `name` (hard rule) | prod 1-6 released 2026-09-23 (db/ and data/ identical to 1-5); consumer break: `name` |
...
| `thorlabsMcls1` | CHANGED | thorlabsMcls1 0-4 | sim/ (3 files); `name` param removed (`thorlabsMcls1`); `thorlabsMcls1` params removed: `desc`; databases args give comment-only macros an empty value (`desc`) | publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; no template uses `name` (hard rule); used only in template comments; no `macLib: macro ... is undefined` at boot | consumer break: `name`; pending user: dropped `desc` (consumer break) |
| `ttiCPX` | CHANGED | ttiCPX 3-0 | docs/ (1 file); +1 template(s); +entities `ttiCPXChannel`; +1 .req; databases args give comment-only macros an empty value (`name`) | publishable upstream extras, kept out of vendoring by `ibek.manifest.yaml`; release templates not covered on main; autosave tags in the templates; no `macLib: macro ... is undefined` at boot | - |
...
| `ttiMX` | NEW | ttiMX 1-4 | 2 templates, 1 protocol file(s), 1 .req; entities `ttiMX_channel`, `ttiMX_general` | StreamDevice module that passes the runtime gate, not on main | prod 1-5 released 2026-09-23 (db/ and data/ identical to 1-4) |
